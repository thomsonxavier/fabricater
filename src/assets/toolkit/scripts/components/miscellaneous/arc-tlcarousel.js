...
/* Unique Key */
const COMPONENT_KEY = 'arcTlCarousel_key'; //unique key

/* Class */
export class arcTlCarouselElement extends BaseClass {

    constructor(element) {
        super(element);

        this._wrapper = this._element.querySelector('.c-tlslider__container');
        this._prevbtn = this._element.querySelector('.c-tlslider__prevbtn');
        this._nextbtn = this._element.querySelector('.c-tlslider__nextbtn');
        this._container = this._element.querySelector('.c-tlslider__list');
        this._cards = Array.from(this._container.querySelectorAll('.c-tlslider__item')) || [];
    ...
    _setPosition(index){
        var upperlimit = Math.floor(this._cards.length / 2);
    ...
    _moveToTarget(pos, item){
        ...
        var sliderGap = parseInt(window.getComputedStyle(this._container).getPropertyValue('column-gap')) || 32;
    ...
    static initInterface(element) {
        ...
        return new arcTlCarouselElement(element);
    }
}


// Function to create instances
export const arcTlCarouselINIT = () =>{
    // NOTE: CALL THIS ON ngAfterViewInit()
    let arcTlCarouselEls = [].slice.call(document.querySelectorAll('.c-tlslider__wrap')) || [];
    let elements = arcTlCarouselEls.map(item => arcTlCarouselElement.initInterface(item));
