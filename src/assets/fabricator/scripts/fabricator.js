...
import { leftMenuTabs } from "./left-menu-tabs";
import { versionClick } from "./versionClick";
...
if (fabricator.test.sessionStorage) {
  sessionStorage.fabricator =
    sessionStorage.fabricator || JSON.stringify(fabricator.options);
}
...
fabricator.getOptions = () => {
  return fabricator.test.sessionStorage
    ? JSON.parse(sessionStorage.fabricator)
    : fabricator.options;
};
...
fabricator.menuToggle = () => {
  ...
  // const menutabs = document.querySelectorAll(".f-menu-tabs__heading");

  // const menuClick = (menutab) => {
  //   const subMenu = menutab.nextElementSibling;
  //   if (subMenu) {
  //     subMenu.classList.toggle("-active");
  //     menutab.classList.toggle("-active");
  //   }
  //   const elemID = menutab.getAttribute("data-menu-tab");
  //   if (elemID && !menutab.classList.contains("-children")) {
  //     const groupElements = document.querySelectorAll(".f-item-group");
  //     for (let i = 0; i < groupElements.length; i++) {
  //       groupElements[i].classList.remove("-active");
  //     }
  ...
  //   menutabs[i].addEventListener("click", menuClick.bind(this, menutabs[i]));
  // }
...
fabricator.allItemsToggles = () => {
  ...
  const toggleAllControls = document.querySelectorAll(
    ".f-controls [data-f-toggle-control]"
  );
  const options = fabricator.getOptions();
  ...
  const toggleAllItems = (type, value) => {
    const button = document.querySelector(
      `.f-controls [data-f-toggle-control=${type}]`
    );
...
fabricator.singleItemToggle = () => {
  const itemToggleSingle = document.querySelectorAll(
    ".f-item-group [data-f-toggle-control]"
  );
  ...
  const toggleSingleItemCode = (e) => {
    ...
    const type = e.currentTarget.getAttribute("data-f-toggle-control");
    group
      .querySelector(`[data-f-toggle=${type}]`)
      .classList.toggle("f-item-hidden");
  };
...
fabricator.bindCodeAutoSelect = () => {
  const codeBlocks = document.querySelectorAll(".f-item-code pre");
...
fabricator.codeBlocks = () => {
  const codeBlocks = document.querySelectorAll(".f-item-code-blocks__tab");

  const removeActiveElements = (element) => {
  ...
  const select = (block) => {
    const tabs = block.parentElement;
    const contentBlocks = block.parentElement.nextElementSibling;
    const codeType = block.getAttribute("data-toggle");

    const contentElem = contentBlocks.querySelector(`.${codeType}`);
    removeActiveElements(tabs);
    removeActiveElements(contentBlocks);
